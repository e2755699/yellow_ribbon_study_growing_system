...
- 每日出席／每日表現返回時**不詢問、直接自動保存是刻意的產品設計**（2026-09-26 使用者確認）：修改只留在本機草稿，離開或切換篩選時才寫入一次，以節省 Firestore 寫入次數，也免去每次返回再按儲存。巡檢時曾誤判為 bug 並加上詢問（`fd0250c`、`f670f17`），已還原並改為明確的 `showSaveConfirmation: false` 加註解。瀏覽器測試期間因此寫入一筆測試出席（2026-09-26 台南永康區，謝家豪「出席」），已改回「缺席」並重新載入確認。
