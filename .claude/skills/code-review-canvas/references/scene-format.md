...
pinch, keyboard, click-to-isolate focus, the readout panel, PNG export, the
light/dark toggle, and a shareable URL hash. You author only a **scene**: a
JavaScript fragment defining nine constants. `scripts/build.js` splices it into
the template.
...
- `C` — the fixed dark palette (`C.bg C.boxFill C.boxStroke C.bandFill C.line
  C.sky C.orange C.slate C.ink C.mut C.dim C.plane C.arrowLbl`) — plus
  `C.rose`, which is reserved and must not be used (see PLANES below)
- `planeColor(id)` — returns the hex colour of a plane

`C` always holds the **dark** values, in both themes. Light mode is a paint-time
remap, not a swap of `C`, because a scene bakes colours in when it is evaluated
(`SWATCHES` takes `C.boxStroke`, `TEXTS` runs take literal hex) and a mutated
`C` would leave those stale. So author against the dark palette and let `hue()`
handle the rest — never branch on the theme inside a scene.
...
Recommended hues: slate `#94A3B8`, sky `#7DD3FC`, violet `#A78BFA`, cyan
`#22D3EE`, green `#4ADE80`, orange `#F6821F`, amber `#FBBF24`. Push hues furthest
apart where columns sit **side by side** — adjacent hues in adjacent columns is
the one combination that reads badly. 4–7 planes is the sweet spot.

**The list is a hard constraint, not taste.** These seven are tuned for the dark
background, and light mode remaps them at paint time through a fixed dark→light
table in the template (`LIGHT_HUE`). A hue outside the list has no entry, so it
survives into light mode unchanged and reads washed out on the pale background.
`validate.js` warns when a plane hue is off-palette. If you genuinely need a new
hue, add both halves — the plane list here *and* the `LIGHT_HUE` entry.

The same applies to any raw hex a scene writes directly: `b.stroke` on a neutral
band, a `runs[].fill` in the strapline, a `SWATCHES` stroke. Prefer `C.*` and
`planeColor()`, which are already mapped; the greys the canonical example uses
(`#3A4250`, `#4A5462`, `#C9D2DC`) are mapped too.

**Feature toggles take no hue of their own.** Mark them with a `⏻ ` prefix on
the box title and let the box sit in its band's plane — see SKILL.md. Colour
answers "which layer", and only that.

### Title prefixes

Two markers, both carried on the `bl` title so they need no template support
and cost one glyph of width each. Every one used must appear in the legend.

One marker only: `⏻ ` for a feature toggle, carried on the `bl`
title so it needs no template support. It is present in 130+ system fonts, so it
never renders as tofu.

Resist adding more. Reading-order badges were tried and removed (see SKILL.md);
`⚠` and `✎` are unavailable in practice (34 and 8 fonts) and would fall back to
colour emoji, smuggling hue back in. A canvas that needs a second prefix is a
canvas explaining too much in the margin.

The validator measures these at 0.6em while most faces render them full-width,
so it under-measures a prefixed title by a few px. Leave slack on the longest
titled box and confirm visually.

### Legend rows with glyph captions

The `⏻` caption is a swatch-less row that sits **between** the two line-style
swatches and the plane rows, which pushes the plane rows and `CHIPS` down. With
`g` glyph rows (normally 1, or 0 when the scene has no toggle):

- glyph captions at `y = 134 + i * 24` for i in 0…g−1
- plane captions start at `y = 134 + g * 24`
- `CHIPS` at `y = 123 + (g + i) * 24`
- the legend ends at `y = 134 + (g + planes − 1) * 24`, and the first band needs
  ~46px below that

Budget this before laying out bands: one glyph row plus six planes ends the
legend at y = 278 and pushes the first band to y ≈ 330.

**Red is reserved — never assign it to a plane.** Rose `#FB7185` and every
other red stay out of the plane palette so red carries no architectural
meaning in these canvases and is free for a separate purpose. `C.rose` remains
in the template palette for that purpose; a scene must not reach for it. Seven
non-red hues are listed above, which is the whole plane budget — if you find
yourself wanting an eighth colour, merge two planes instead.
...
**Fitting text**: JetBrains Mono advances 0.6 × font-size per Latin character,
but CJK glyphs are full-width at 1.0 × font-size. Per character:

| | `bs` (11px) | `bl` (13.5px) | `bn` (10.5px) |
|---|---|---|---|
| Latin / digits | 6.6px | 8.1px | 6.3px |
| Chinese | 11px | 13.5px | 10.5px |

So a 380-wide box holds ~52 Latin characters on a `bs` line but only ~31
Chinese ones — Chinese body lines have to be noticeably terser. Required box
width ≥ 24 (left pad) + longest line width + 12 (right pad). The validator
measures per character and errors on overflow — prefer splitting a line over
widening past the band. Because mixed CJK/Latin lines are not strictly
monospaced (different faces), leave a few px of slack and confirm visually.
...
const TEXTS = [
  ...
  // One caption per row, 24px apart: the two line-style swatches first, then
  // ONE ROW PER PLANE — never a single "colour — the plane" catch-all.
  { s: 'legend', x: 1082, y: 86,  t: 'dashed — an abstraction we own' },
  { s: 'legend', x: 1082, y: 110, t: 'solid — swappable infra & app surfaces' },
  // a swatch-less row for the toggle glyph, only when the scene has toggles;
  // it pushes the plane rows (and CHIPS) down by 24
  { s: 'legend', x: 1082, y: 134, t: '⏻ — feature toggle' },
  { s: 'legend', x: 1082, y: 158, t: 'Humans & connected systems' },
  { s: 'legend', x: 1082, y: 182, t: 'Product — apps/web' },
  { s: 'legend', x: 1082, y: 206, t: 'Execution plane' },
...
// One chip per plane, pipeline order, STACKED VERTICALLY 24px apart so each
// chip lines up with its own caption row above. Never lay them out
// horizontally — a row of chips can only carry one shared caption, which
// leaves the reader guessing which colour means which plane.
// y: 123 + (g + i) * 24, where g is the number of glyph caption rows above
const CHIPS = ['humans', 'product', 'execution']
  .map((id, i) => ({ x: 1046, y: 123 + (1 + i) * 24, w: 26, h: 13, fill: planeColor(id) }));
```

**Budget vertical room for the legend before placing the first band.** With
`n` planes the legend runs to `y = 134 + (n − 1) × 24`; the first band must
start at least ~46px below that. Six planes push the first band to y ≈ 300
rather than the usual 200 — shift every band, box, edge and label down to
match, and grow `H` by the same amount.
