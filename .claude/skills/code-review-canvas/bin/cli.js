#!/usr/bin/env node
// draw-architecture — installs the code-review skill into an agent
// skills directory.
//
// The npm package is named after the repo (draw-architecture); the skill it
// installs is named code-review, which is what SKILL.md declares and
...
//   skill       a hand-installed code-review (SKILL.md matches)
...
function inspect(dest) {
  ...
  try {
    ...
    if (/^name:[ \t]*code-review[ \t]*$/m.test(skillMd)) return { kind: 'skill', symlink };
  } catch { /* fall through */ }
...
async function clearSite(dest, { force, assumeYes }) {
  ...
  if (found.kind === 'foreign') {
    if (!force) {
      console.error(`error: ${tilde(dest)} exists but is not an code-review skill.`);
    ...
    console.log(`  found a hand-installed code-review at ${tilde(dest)}`);
    if (!(await confirm('  replace it?', assumeYes || force))) return false;
  }
...
async function uninstall(opts) {
  ...
  for (const dest of skipped) {
    console.warn(`! ${tilde(dest)} is not an code-review skill — leaving it alone`);
  }
