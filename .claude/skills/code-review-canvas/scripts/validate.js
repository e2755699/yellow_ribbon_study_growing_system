...
// Light mode remaps the palette at paint time from a fixed dark→light table, so a
// plane hue outside the recommended list survives into light mode unchanged and
// reads washed out on the pale background. A warning, not an error: an off-palette
// hue is still a legitimate choice for a canvas only ever read dark.
const LIGHT_MAPPED = new Set(
  ['#94A3B8', '#7DD3FC', '#A78BFA', '#22D3EE', '#4ADE80', '#F6821F', '#FBBF24',
   '#FB7185', '#3A4250'].map(h => h.toUpperCase()));
for (const [id, p] of Object.entries(PLANES)) {
  if (!LIGHT_MAPPED.has(String(p.c || '').toUpperCase())) {
    warn(`plane "${id}" hue ${p.c} is outside the recommended palette, so light mode cannot ` +
         `remap it — pick one of slate #94A3B8, sky #7DD3FC, violet #A78BFA, cyan #22D3EE, ` +
         `green #4ADE80, orange #F6821F, amber #FBBF24`);
  }
}
...
// CJK glyphs are full-width (~1.0em) where Latin in JetBrains Mono is 0.6em, and
// they fall back to a system face (PingFang / Noto) that JetBrains Mono has no
// glyphs for. Counting every char at 0.6em under-measures a Chinese line by ~67%
// — it passes validation and then overflows its box on screen. Measure per char.
const FULLWIDTH = /[ᄀ-ᅟ⺀-〾ぁ-㏿㐀-䶿一-鿿ꀀ-꓏가-힣豈-﫿︰-﹏＀-｠￠-￦]/;
function textWidth(styleId, t) {
  const st = STYLE[styleId] || { size: 11, ls: 0 };
  let w = 0;
  for (const ch of t) w += st.size * (FULLWIDTH.test(ch) ? 1.0 : 0.6) + st.ls;
  return w;
}
