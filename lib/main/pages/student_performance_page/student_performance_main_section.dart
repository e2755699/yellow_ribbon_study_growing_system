...
class _StudentPerformanceMainSectionState
  ...
  Widget build(BuildContext context) {
    ...
    final emptyMessage = widget.customTitles?['emptyMessage'] ?? '找不到該學生近一個月的表現記錄';
    
    // 使用自定义标题时，不过滤记录
    final recordsToShow = widget.customTitles != null ? widget.records : filteredRecords;
    ...
    final classLocation = widget.studentDetail?.classLocation ?? 
                         (filteredRecords.isNotEmpty ? filteredRecords[0].classLocation.name : '');
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            InfoCardLayoutWith1Column(
              title: recordsTitle,
              titleSuffix: widget.studentId.isNotEmpty ? IconButton(
  ...
  Widget _buildPerformanceRatingDropdown(StudentDailyPerformanceRecord record) {
    return DropdownButton<PerformanceRating>(
      ...
      items: PerformanceRating.values.map<DropdownMenuItem<PerformanceRating>>((PerformanceRating value) {
  ...
  Widget _buildRemarksTextField(StudentDailyPerformanceRecord record) {
    final controller = TextEditingController(text: record.remarksNotifier.value);
  ...
  Widget _buildPerformanceStats(BuildContext context, List<StudentDailyPerformanceRecord> records) {
    // 计算各项统计数据
    int totalRecords = records.length;
    int homeworkCompletedCount = records
        .where((record) => record.homeworkCompletedNotifier.value)
        .length;
    int helperCount = records
        .where((record) => record.isHelperNotifier.value)
        .length;
